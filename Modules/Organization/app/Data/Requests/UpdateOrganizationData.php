...
class UpdateOrganizationData extends Data
{
    public function __construct(
        ...
        #[Nullable, StringType, Max(255), Regex('/^[a-z0-9\-]+$/')]
        public readonly ?string $slug,

        #[Nullable, StringType, Max(30), Regex('/^[A-Z0-9\-]+$/')]
    ...
    {
        $currentId = request()->route('organization')?->id;

        return [
            'slug' => [
                'nullable', 'string', 'max:255', 'regex:/^[a-z0-9\-]+$/',
                Rule::unique('organizations', 'slug')->ignore($currentId),
            ],
            'code' => [
                'nullable', 'string', 'max:30', 'regex:/^[A-Z0-9\-]+$/',
                Rule::unique('organizations', 'code')->ignore($currentId),
            ],
        ];
    }
}
