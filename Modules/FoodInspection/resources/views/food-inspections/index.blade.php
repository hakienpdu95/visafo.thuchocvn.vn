...
@section('title', 'Sổ kiểm thực Bước 1')

@section('content')
<div x-data="foodInspectionListPage({{ Js::from([
    'apiUrl'     => route('backend.api.food-inspections'),
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <p class="text-sm text-base-content/50 mt-0.5">Kiểm tra thực phẩm trước khi chế biến (QĐ 1246/QĐ-BYT)</p>
        </div>
        <div class="flex items-center gap-2">
            ...
            @can('create', \Modules\FoodInspection\Models\FoodInspectionStep1Log::class)
            <a href="{{ route('backend.food-inspections.create') }}" class="btn btn-primary btn-sm gap-1.5">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="section-page">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3 items-end">
                    ...
                    <div class="form-control lg:col-span-2">
                        <label class="label mb-2">
                            ...
                            <span class="label-text-alt text-xs text-base-content/40">Khách hàng, địa điểm, NCC, người giao, thực phẩm</span>
                        </label>
                    ...
                    <div class="form-control">
                        <label class="label py-0.5">
                            <span class="label-text text-xs font-medium">Người kiểm tra</span>
                        </label>
                        <select id="ts-inspector" x-model="filters.inspector" @change="onFilterChange()"
                                data-ts-placeholder="Tất cả người kiểm tra"
                                class="select select-sm select-bordered w-full">
                            <option value="">Tất cả</option>
                            @foreach($inspectors as $inspector)
                            <option value="{{ $inspector['value'] }}">{{ $inspector['text'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-control">
                        <label class="label py-0.5">
                            <span class="label-text text-xs font-medium">Nhóm thực phẩm</span>
                        </label>
                        <select id="ts-food-group" x-model="filters.foodGroup" @change="onFilterChange()"
                                data-ts-placeholder="Tất cả nhóm thực phẩm"
                                class="select select-sm select-bordered w-full">
                            <option value="">Tất cả</option>
                    ...
                    <div class="form-control">
                        <label class="label py-0.5">
                            <span class="label-text text-xs font-medium">Kết quả</span>
                        </label>
                        <select id="ts-result" x-model="filters.result" @change="onFilterChange()"
                                data-ts-placeholder="Tất cả kết quả"
                                class="select select-sm select-bordered w-full">
                            <option value="">Tất cả</option>
        ...
        <div class="card">
            <div class="card-body p-0 overflow-hidden tabulator-daisy">
                <div id="food-inspection-table"></div>
            </div>
        </div>
    </div>
</div>
