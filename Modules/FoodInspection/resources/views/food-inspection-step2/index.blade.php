...
@section('content')
<div x-data="step2ListPage({{ Js::from([
    'apiUrl'    => route('backend.api.food-inspection-step2'),
    'customers' => $customers,
    'results'   => [['value' => 'passed', 'text' => 'Đạt toàn bộ'], ['value' => 'failed', 'text' => 'Có món không đạt']],
]) }})">

    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <p class="text-sm text-base-content/50 mt-0.5">Kiểm tra khi chế biến — Mẫu số 2 (QĐ 1246/QĐ-BYT)</p>
        </div>
        <div class="flex items-center gap-2">
            ...
            @can('create', \Modules\FoodInspection\Models\FoodInspectionStep2Log::class)
            <a href="{{ route('backend.food-inspection-step2.create') }}" class="btn btn-primary btn-sm gap-1.5">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
                Kiểm thực Bước 2 mới
            </a>
            @endcan
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
                    ...
                    <div class="form-control lg:col-span-2">
                        <label class="label mb-2">
                            ...
                            <span class="label-text-alt text-xs text-base-content/40">Cơ sở, địa điểm, món ăn, nguyên liệu</span>
                        </label>
                    ...
                    <div class="form-control">
                        ...
                        <select id="ts-result" x-model="filters.result" @change="onFilterChange()"
                                ...
                                class="select select-sm select-bordered w-full">
                            <option value="">Tất cả</option>
                            <option value="passed">Đạt toàn bộ</option>
                            <option value="failed">Có món không đạt</option>
                        </select>
                    </div>
