...
@section('title', 'Kiểm thực Bước 2 ' . $log->inspection_date?->format('d/m/Y'))
...
@php
    $rows = $log->details->values()->map(fn ($d, $i) => [
        ...
        'quantity'          => $d->quantity,
        'prep_time'         => $d->prep_time ? substr($d->prep_time, 0, 5) : null,
        'cook_time'         => $d->cook_time ? substr($d->cook_time, 0, 5) : null,
...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-base-content">Kiểm thực Bước 2 — {{ $log->inspection_date?->format('d/m/Y') }}</h1>
        <p class="text-sm text-base-content/50 mt-0.5">{{ $log->customer_name }}</p>
    </div>
    <div class="flex items-center gap-2">
        <a href="{{ route('backend.food-inspection-step2.index') }}" class="btn btn-ghost btn-sm">Danh sách</a>

        @can('update', $log)
        <a href="{{ route('backend.food-inspection-step2.edit', $log) }}" class="btn btn-primary btn-sm gap-1.5">
        ...
        @can('delete', $log)
        <button type="button" class="btn btn-error btn-sm gap-1.5" onclick="document.getElementById('fiStep2DeleteModal').showModal()">
...
<div class="flex flex-col gap-6">
    ...
    <div>
        <p class="text-xs font-semibold text-base-content/40 uppercase tracking-wide mb-2">Kiểm tra khi chế biến ({{ count($rows) }} món)</p>
        <div class="section-page">
            <div class="card">
                <div class="card-body p-0 overflow-hidden tabulator-daisy">
                    <div id="step2-detail-table" data-rows="{{ json_encode($rows, JSON_UNESCAPED_UNICODE) }}"></div>
                </div>
            </div>
        </div>
    </div>
...
@can('delete', $log)
<dialog id="fiStep2DeleteModal" class="modal">
    <div class="modal-box max-w-sm">
        <h3 class="font-bold text-lg text-error">Xác nhận xóa</h3>
        <p class="py-3 text-sm text-base-content/70">
            Bạn có chắc muốn xóa sổ kiểm thực Bước 2 ngày <strong class="text-base-content">{{ $log->inspection_date?->format('d/m/Y') }}</strong>
            của {{ $log->customer_name }} cùng {{ count($rows) }} dòng món ăn?
        </p>
        <form method="POST" action="{{ route('backend.food-inspection-step2.destroy', $log) }}" class="modal-action mt-4">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-error btn-sm">Xóa</button>
            <button type="button" class="btn btn-ghost btn-sm" onclick="document.getElementById('fiStep2DeleteModal').close()">Hủy</button>
        </form>
    </div>
