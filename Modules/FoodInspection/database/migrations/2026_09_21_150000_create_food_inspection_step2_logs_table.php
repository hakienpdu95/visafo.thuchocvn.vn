    ...
    public function up(): void
    {
        ...
        Schema::create('food_inspection_step2_logs', function (Blueprint $table) {
            ...
            $table->foreignUlid('customer_id')->constrained('customers')->restrictOnDelete()
                ->comment('Cơ sở / doanh nghiệp suất ăn được kiểm thực (Master Data Khách hàng)');
            ...
            $table->foreignUlid('delivery_point_id')->nullable()->constrained('customer_delivery_points')->nullOnDelete()
                ->comment('Điểm giao/bếp ăn khớp theo tên địa điểm (nếu có)');
            ...
            $table->unsignedSmallInteger('failed_items_count')->default(0)
                ->comment('Số món có tiêu chí vệ sinh/cảm quan không đạt — tính lúc lưu để lọc nhanh');
