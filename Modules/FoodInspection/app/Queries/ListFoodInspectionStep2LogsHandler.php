...
class ListFoodInspectionStep2LogsHandler implements QueryHandlerInterface
{
    ...
    {
        /** @var ListFoodInspectionStep2LogsQuery $query */
        $sortField = in_array($query->sortField, self::SORTABLE, true) ? $query->sortField : 'inspection_date';
        $sortDir = $query->sortDir === 'asc' ? 'asc' : 'desc';

        $q = FoodInspectionStep2Log::query()->visibleTo()->withCount('details');

        if ($query->search) {
            $term = '%' . $query->search . '%';
            $q->where(fn (Builder $s) => $s->where('customer_name', 'like', $term)->orWhere('location_name', 'like', $term)
                ->orWhereHas('details', fn (Builder $d) => $d->where('dish_name', 'like', $term)->orWhere('main_ingredients', 'like', $term)));
        }
        if ($query->customerId) {
