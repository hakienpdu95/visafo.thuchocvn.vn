...
function buildColumns(canDelete) {
    return [
        {
            title: 'Mã', field: 'code', width: 160, sorter: 'string', frozen: true,
            formatter(cell) {
                return '<span class="font-mono text-xs">' + esc(cell.getValue()) + '</span>';
            },
        },
        {
            title: 'Tên nhóm', field: 'name', minWidth: 220, sorter: 'string',
            formatter(cell) {
        ...
        {
            title: 'Mô tả', field: 'description', minWidth: 220, headerSort: false,
            formatter(cell) {
                return esc(cell.getValue()) || '<span class="text-base-content/25 text-xs">—</span>';
            },
        },
        {
            title: 'Trạng thái', field: 'status_label', width: 140, hozAlign: 'center', sorter: 'string',
            formatter(cell) {
                const d = cell.getRow().getData();
            ...
            formatter(cell) {
                ...
                if (canDelete && d.can_delete) {
                    html += '<button class="btn btn-ghost btn-xs btn-square text-error/30 hover:text-error" title="Xóa"'
                        ...
                        + ' onclick="window.categoryDeleteConfirm(this.dataset.url, this.dataset.name)">'
...
window.categoryDeleteConfirm = function (url, name) {
    ...
    confirmBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            if (res.ok) {
                ...
                window.categoryTable?.replaceData();
            } else {
...
const LS_COLS = 'category-list-hidden-cols';

document.addEventListener('alpine:init', () => {

    Alpine.data('categoryListPage', (serverData = {}) => {
        const {
            apiUrl    = '',
            statuses  = [],
            canDelete = false,
        ...
        return {
            ...
            get toggleableCols() {
                return COLUMNS
                    .filter(c => c.field !== 'id' && c.field !== 'code')
            ...
            _setup() {
                ...
                tableInst = new window.Tabulator('#category-table', {
                    ajaxURL:    apiUrl,
                    ...
                    ajaxError: (error) => console.error('[category] API error', error),
                    ...
                    placeholder: '<div class="py-16 text-center opacity-40">'
                        ...
                        + '<p class="text-sm">Không có nhóm hàng nào</p></div>',
                });

                window.categoryTable = tableInst;
