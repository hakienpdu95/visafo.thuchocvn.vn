...
function buildColumns(canDelete) {
    return [
        {
            title: 'Hàng hóa NCC', field: 'name', minWidth: 220, sorter: 'string', frozen: true,
            formatter(cell) {
                const d = cell.getRow().getData();
                return '<div>'
                    + '<a href="' + esc(d.show_url) + '" class="font-semibold text-sm hover:text-primary transition-colors">' + esc(d.name) + '</a>'
                    + '<p class="text-xs text-base-content/40 font-mono mt-0.5">' + esc(d.vendor_sku || '—') + '</p>'
                    + '</div>';
            },
        },
        {
        ...
        {
            title: 'Ánh xạ sản phẩm chuẩn', field: 'product_name', minWidth: 220, headerSort: false,
            formatter(cell) {
                const d = cell.getRow().getData();
                if (!d.product_name) return '<span class="text-base-content/25 text-xs">Chưa ánh xạ</span>';
                return '<span class="text-sm">' + esc(d.product_name) + '</span>'
                    + '<span class="text-xs text-base-content/40 font-mono ml-1">' + esc(d.product_sku || '') + '</span>';
            },
        },
        {
            ...
            formatter(cell) {
                ...
                if (canDelete && d.can_delete) {
                    html += '<button class="btn btn-ghost btn-xs btn-square text-error/30 hover:text-error" title="Xóa"'
                        ...
                        + ' onclick="window.partnerProductDeleteConfirm(this.dataset.url, this.dataset.name)">'
...
window.partnerProductDeleteConfirm = function (url, name) {
    ...
    confirmBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            if (res.ok) {
                ...
                window.partnerProductTable?.replaceData();
            } else {
                alert(data.message || 'Xóa thất bại. Vui lòng thử lại.');
            }
        } catch (e) {
            console.error('[partner-product] delete failed', e);
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('partnerProductListPage', (serverData = {}) => {
        const {
            apiUrl    = '',
            statuses  = [],
            canDelete = false,
        ...
        return {
            ...
            _setup() {
                ...
                tableInst = new window.Tabulator('#partner-product-table', {
                    ajaxURL:    apiUrl,
                    ...
                    ajaxError: (error) => console.error('[partner-product] API error', error),
                    ...
                    placeholder: '<div class="py-16 text-center opacity-40">'
                        ...
                        + '<p class="text-sm">Không có hàng hóa NCC nào</p></div>',
                });

                window.partnerProductTable = tableInst;
            },
