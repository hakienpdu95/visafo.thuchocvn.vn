...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-base-content">Chỉnh sửa thương hiệu</h1>
        <p class="text-sm text-base-content/50 mt-0.5">{{ $brand->name }}</p>
    </div>
...
<form method="POST" action="{{ route('backend.brands.update', $brand) }}" novalidate data-brand-form>
    @csrf
    @method('PUT')
        ...
        <div class="card bg-base-100 shadow-sm border border-base-200">
            <div class="card-body">
                ...
                <div class="form-control mb-4">
                    ...
                    @if($brand->getFirstMediaUrl('logo', 'medium'))
                    <div class="flex items-center gap-3 mb-3">
                        <img src="{{ $brand->getFirstMediaUrl('logo', 'medium') }}" alt="{{ $brand->name }}"
                ...
                <div class="form-control">
                    ...
                    <textarea name="description" rows="4"
                              ...
                              placeholder="Giới thiệu ngắn gọn về thương hiệu...">{{ old('description', $brand->description) }}</textarea>
