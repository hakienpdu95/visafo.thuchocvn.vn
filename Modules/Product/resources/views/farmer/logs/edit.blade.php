...
<div x-data="{
        steps: {{ Js::from($steps) }},
        selectedKey: {{ Js::from($currentKey) }},
    ...
    <form method="POST" action="{{ route('farmer.logs.update', $farmingLog) }}" enctype="multipart/form-data" class="space-y-4" data-farming-log-form>
        @csrf
        @method('PUT')
        ...
        <div class="form-control">
            ...
            <div class="flex gap-2">
                <input type="text" id="fp-activity_date" name="activity_date" value="{{ $farmingLog->activity_date->format('Y-m-d H:i:s') }}"
            ...
            <div class="space-y-4">
                <div class="form-control">
                    ...
                    <select id="ts-agri_fertilizer_id" name="agri_fertilizer_id" class="select select-bordered w-full ts-init"
                        ...
                        <option value="{{ $fertilizer->id }}" {{ $farmingLog->agri_fertilizer_id === $fertilizer->id ? 'selected' : '' }}>{{ $fertilizer->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div class="form-control">
                        ...
                        <input type="number" step="0.01" name="quantity" value="{{ $farmingLog->quantity }}" class="input input-bordered w-full">
                    </div>
                    <div class="form-control">
                        <label class="label py-0 pb-1.5"><span class="label-text font-medium">Đơn vị</span></label>
                        <input type="text" name="unit" value="{{ $farmingLog->unit }}" placeholder="kg, lít..." class="input input-bordered w-full">
                    </div>
                </div>
                <div class="form-control">
                    <label class="label py-0 pb-1.5"><span class="label-text font-medium">Cách bón</span></label>
                    <input type="text" name="method_or_target" value="{{ $farmingLog->method_or_target }}" placeholder="VD: bón lót, bón thúc" class="input input-bordered w-full">
                </div>
            </div>
        </template>
            ...
            <div class="space-y-4">
                <div class="form-control">
                    ...
                    <select id="ts-agri_pesticide_id" name="agri_pesticide_id" class="select select-bordered w-full ts-init"
                        ...
                        <option value="{{ $pesticide->id }}" {{ $farmingLog->agri_pesticide_id === $pesticide->id ? 'selected' : '' }}>{{ $pesticide->trade_name }}@if($pesticide->target_pest) ({{ $pesticide->target_pest }})@endif</option>
                        @endforeach
                    </select>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="form-control">
                        <label class="label py-0 pb-1.5"><span class="label-text font-medium">Đơn vị</span></label>
                        <input type="text" name="unit" value="{{ $farmingLog->unit }}" placeholder="ml, lít..." class="input input-bordered w-full">
                    </div>
                </div>
                <div class="form-control">
                    <label class="label py-0 pb-1.5"><span class="label-text font-medium">Đối tượng phòng trừ</span></label>
                    <input type="text" name="method_or_target" value="{{ $farmingLog->method_or_target }}" placeholder="VD: rệp, sâu vẽ bùa" class="input input-bordered w-full">
                </div>
            </div>
        </template>
        ...
        <template x-if="activityType === 'harvest'">
            <div class="grid grid-cols-2 gap-3">
                ...
                <div class="form-control">
                    <label class="label py-0 pb-1.5"><span class="label-text font-medium">Đơn vị</span></label>
                    <input type="text" name="unit" value="{{ $farmingLog->unit }}" placeholder="kg, tấn..." class="input input-bordered w-full">
                </div>
            </div>
        </template>
        ...
        <div class="form-control">
            ...
            <textarea name="notes" rows="2" class="textarea textarea-bordered w-full">{{ $farmingLog->notes }}</textarea>
        </div>

        <div class="form-control">
            <label class="label py-0 pb-1.5"><span class="label-text font-medium">Ảnh minh chứng</span></label>
            @if($farmingLog->image_path)
            <img src="{{ \Illuminate\Support\Facades\Storage::url($farmingLog->image_path) }}" class="mb-2 rounded-lg border border-base-200 max-h-48 object-contain">
            @endif
            <label class="btn btn-outline btn-block gap-2">
                ...
                {{ $farmingLog->image_path ? 'Thay ảnh khác' : 'Chụp ảnh minh chứng' }}
