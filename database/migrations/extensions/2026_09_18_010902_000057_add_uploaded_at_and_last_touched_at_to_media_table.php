...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('media', function (Blueprint $table) {
            if (!Schema::hasColumn('media', 'uploaded_at')) {
                $table->timestamp('uploaded_at')->nullable()->comment('Timestamp user thực sự upload — bảo toàn khi migrate từ bảng cũ');
            }
    ...
    {
        Schema::table('media', function (Blueprint $table) {
            $cols = array_filter(['uploaded_at', 'last_touched_at'], fn($c) => Schema::hasColumn('media', $c));
